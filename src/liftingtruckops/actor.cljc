(ns liftingtruckops.actor
  "LiftingTruckOpsActor -- the ISCO-08 8344 lifting truck operators
  warehouse-crew scheduling/logistics coordination actor as a
  `langgraph.graph/state-graph` (ADR-2607011000 / ADR-2607121000 /
  CLAUDE.md Actors section). One graph run = one coordination
  ...
  The unconditional invariant: the Warehouse Logistics Advisor can
  never directly commit a record the LiftingTruckOpsGovernor refuses
  -- every commit-record! call is gated behind `:decide`. This actor
  coordinates SCHEDULING/LOGISTICS ONLY -- it never operates the
  lifting truck and never finalizes a lift-operation/load-movement
  decision; that stays with the human operator on site."
...
(defn build-graph
  "Build a compiled LiftingTruckOpsActor graph. `store` implements
  `liftingtruckops.store/Store`. `advisor` implements
  `liftingtruckops.advisor/Advisor` (defaults to `mock-advisor`).
